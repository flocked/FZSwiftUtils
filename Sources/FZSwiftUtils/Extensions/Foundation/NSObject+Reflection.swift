...
public extension NSObject {
    static func classReflection(includeSuperclass: Bool = false, excludeReadOnlyProperties: Bool = false) -> Dictionary<String, Any>? {
    ...
    static func containsProperty(_ propertyName: String) -> Bool {
        return self.typeOfProperty(propertyName) != nil
    }
    
    func classReflection() -> Dictionary<String, Any>? {
    ...
    func containsProperty(named propertyName: String) -> Bool {
        return self.typeOfProperty(named: propertyName) != nil
    }
}

internal func getTypesOfProperties(in clazz: NSObject.Type, includeSuperclass: Bool = false, excludeReadOnlyProperties: Bool = false) -> Dictionary<String, Any>? {
    let types: Dictionary<String, Any> = [:]
    return getTypesOfProperties(in: clazz, types: types, includeSuperclass: includeSuperclass)
}

internal func getTypesOfProperties(in clazz: NSObject.Type, types: Dictionary<String, Any>, includeSuperclass: Bool, excludeReadOnlyProperties: Bool = false) -> Dictionary<String, Any>? {
    var count = UInt32()
...
internal func getTypesOfProperties(ofObject object: NSObject) -> Dictionary<String, Any>? {
...
fileprivate func getTypeOf(property: objc_property_t) -> Any {
    guard let prop = property_getAttributes(property), let attributesAsNSString: NSString = NSString(utf8String: prop) else { return Any.self }
...
fileprivate func isReadOnly(property: objc_property_t) -> Bool {
    guard let prop = property_getAttributes(property), let attributesAsNSString: NSString = NSString(utf8String: prop) else { return false }
    let attributes = attributesAsNSString as String
