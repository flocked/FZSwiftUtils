...
public extension Sequence {
    ...
    func contains<T>(_ keyPath: KeyPath<Element, T?>) -> Bool {
        return self.contains(where: { $0[keyPath: keyPath] != nil })
    }
    
    func first<T>(_ keyPath: KeyPath<Element, T?>) -> T? {
        return self.first(where: { $0[keyPath: keyPath] != nil })?[keyPath: keyPath]
    }
    
    func count<T>(of keyPath: KeyPath<Element, T?>) -> Int {
        return self.filter({ $0[keyPath: keyPath] != nil }).count
    }
    
    func indexes<T>(of keyPath: KeyPath<Element, T?>) -> IndexSet {
        self.indexes(where: { $0[keyPath: keyPath] != nil } )
    }
}
