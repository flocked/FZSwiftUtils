...
internal final class DictionaryKeyedEncodingContainer<Key: CodingKey>:
    ...
    internal var options: DictionaryEncoder.Options {
        container.options
    }
    ...
    internal func encodeNil(forKey key: Key) throws {
        container.collectComponent(encodeNilComponent(at: codingPath + key), forKey: key)
    }

    internal func encode(_ value: Bool, forKey key: Key) throws {
        container.collectComponent(encodeComponentValue(value, at: codingPath + key), forKey: key)
    }
    ...
    internal func encode(_ value: Double, forKey key: Key) throws {
        container.collectComponent(try encodeComponentValue(value, at: codingPath + key), forKey: key)
    }
    ...
    internal func superEncoder() -> Encoder {
        container.superEncoder(forKey: AnyCodingKey.key("super"))
    }
}
