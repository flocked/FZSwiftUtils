...
extension NSObject {
    ...
    static func addHook(_ hook: AnyHook, for selector: Selector) {
        var _hooks = hooks[selector] ?? []
        _hooks.append(hook)
        hooks[selector] = _hooks
    }
    ...
    public func replaceMethod<MethodSignature, HookSignature> (
        ...
        if let klass = self as? AnyClass {
            let hook = try Interpose.ClassHook(class: klass, selector: selector, implementation: implementation).apply()
            Self.addHook(hook, for: selector)
        } else {
           let hook = try Interpose.ObjectHook(object: self, selector: selector, implementation: implementation).apply()
            addHook(hook, for: selector)
        }
