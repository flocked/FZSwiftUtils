...
public extension Sequence where Element: Comparable & Hashable {
    ...
    func sortedByNumberOfOccurences<T>(of keyPath: KeyPath<Element, T?>) -> [Element] {
        let numberOfOccurences = reduce(into: [Element: Int]()) { currentResult, element in
        ...
        return sorted(by: { current, next in numberOfOccurences[current]! < numberOfOccurences[next]! })
    }
