...
public protocol CFType {
    ...
    /// Initializes the Core Foundation value from the specified value if it matches this type.
    init?<V>(_ value: V)
}

public extension CFType {
    /// Returns the value as the specified `Swift` type.
    func asSwift<T: _CFConvertible>() -> T where T._CFType == Self {
        T._bridgeFromCF(self)
    }

    /// Returns the value from the specified `Swift` value.
    static func from<T: _CFConvertible>(_ v: T) -> Self where T._CFType == Self {
        v._bridgeToCF()
    }

    /// Creates the object from the specified `Swift` value.
    init<V: _CFConvertible>(_ value: V) where V._CFType == Self {
...
public extension CFType {
    ...
    static var cfDescription: String {
