...
class HookContext {
    ...
    init(targetClass: AnyClass, selector: Selector, isSpecifiedInstance: Bool) throws {
        ...
        guard let method = ObjCClass(targetClass).method(for: selector, declaredOnly: true)?.method else {
    ...
    func isIMPChanged() throws -> Bool {
        guard let currentMethod = ObjCClass(targetClass).method(for: selector, declaredOnly: true)?.method else {
            throw HookError.internalError(file: #file, line: #line)
        }
